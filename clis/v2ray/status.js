...
cli({
  ...
  description: '查看 VPN 状态：sing-box 路由模式 + TUN 网卡 + 连通性',
  ...
  func: async () => {
    ...
    if (!mode) throw new CommandExecutionError('无法连接 sing-box API (127.0.0.1:10814)', '请确认 v2rayN 正在运行且 sing-box 核心已启动');
    const ad = detectAdapter();
    const net = await testNet('https://www.baidu.com', 5000);
    return [{ mode, adapter: ad ? `${ad.name}/${ad.status}` : 'N/A', net: net.reachable ? 'ok' : 'blocked' }];
  },
});
