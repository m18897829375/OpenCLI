...
cli({
  ...
  description: '开启 VPN 代理（切换 sing-box 路由模式为 Rule）。不重启 v2rayN，即时生效。',
  ...
  func: async () => {
    ...
    if (currentMode !== 'Direct') {
      return [{ action: 'already_on', mode: currentMode, adapter: '—', net: '—' }];
    }
    ...
    if (newMode === 'Rule') {
      return [{ action: 'on', mode: newMode, adapter: ad ? `${ad.name}/${ad.status}` : 'N/A', net: '—' }];
    }
